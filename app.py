...
with st.form("input_form"):
    ...
    # Alias checkbox (with live rerun)
    st.checkbox("Add Aliases", key="add_aliases", on_change=_rerun)
    aliases_input = st.text_input(
        "Aliases (comma-separated)",
        disabled=not st.session_state.add_aliases
    )

    # CORP checkbox (with live rerun)
    st.checkbox("CORP", key="corp", on_change=_rerun)
    service_deliverer = st.text_input(
        "Who delivers the service (e.g. HS PL)",
        disabled=not st.session_state.corp
    )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                output_path = run_generator(
                    ...
                    require_corp=st.session_state.corp,
                    delivering_tag=service_deliverer.upper() if st.session_state.corp else "",
                    support_group=support_input.split("/", 1)[0],
                    managed_by_group=support_input.split("/", 1)[-1],
                    aliases_on=st.session_state.add_aliases,
                    src_dir=Path(tmpdir),
                    out_dir=Path(tmpdir)
                )
