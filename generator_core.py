...
def run_generator(*,
    ...
    def update_commitments(orig):
        ...
        for line in str(orig).splitlines():
            if "RSP" in line and line.strip().startswith("["):
                l=re.sub(r"P1-P4 .*?$",f"P1-P4 {rsp_duration}",line)
                out.append(l)
            elif "RSL" in line and line.strip().startswith("["):
                l=re.sub(r"P1-P4 .*?$",f"P1-P4 {rsl_duration}",line)
                out.append(l)
                ola=l.replace("SLA","OLA",1)
                out.append(ola)
        return "\n".join(out)
    ...
    for wb in src_dir.glob("ALL_Service_Offering_*.xlsx"):
        ...
        if base.empty:
            continue
        row0=base.iloc[0].to_frame().T.copy()
        ...
        if require_corp:
            if country=="DE":
                ...
                has_ds="DS PL" in base["Name (Child Service Offering lvl 1)"].str.cat(sep=" ")
        ...
        pf=str(row0.iloc[0]["Parent Offering"])
        m=re.search(r"\[Parent\s+(.*?)\]",pf,re.I)
        inner=m.group(1).strip() if m else ""
        desc=pf.split("]",1)[-1].strip()
        toks=inner.split()
        ...
        for app in new_apps:
            for recv in receivers:
                ...
                rest=" ".join(t for t in toks if t not in recv.split())
                name_head=f"{head} {rest}]".replace("  "," ").replace(" ]","]")
                new_name=f"{name_head} {desc} {app} Prod {schedule_suffix}".replace("  "," ")
                if new_name in seen:
                    continue
                seen.add(new_name)
    ...
    with pd.ExcelWriter(out,engine="openpyxl") as w:
        for cc,dfc in sheets.items():
            df2=dfc.drop_duplicates(subset=["Name (Child Service Offering lvl 1)"])\
